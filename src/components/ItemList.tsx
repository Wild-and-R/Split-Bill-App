...
export default function ItemList({ initialItems }: ItemListProps) {
  ...
  y += 20;
  ctx.fillText(`Subtotal: Rp ${subtotal.toLocaleString('id-ID')}`, 40, y);
  y += 28;
  ctx.fillText(`Tax (${taxRate}%): Rp ${tax.toLocaleString('id-ID')}`, 40, y);
  y += 28;
  ctx.fillText(
    `Service (${serviceRate}%): Rp ${serviceCharge.toLocaleString('id-ID')}`,
    40,
  ...
  if (splitMode === 'equal') {
    ctx.fillText(
      `Each pays: Rp ${perPersonEqual.toLocaleString('id-ID')}`,
      40,
      y
    );
  } else {
    people.forEach((p, i) => {
      ctx.fillText(
        `${p}: Rp ${perPersonTotals[i].toLocaleString('id-ID')}`,
        40,
        y
      );
      y += 28;
  ...
  ctx.fillText('Generated by Wild-and-R Split Bill', 40, canvas.height - 40);
